...
export default function About() {
  const { hero, stats, originStory, pillars } = aboutPageData;

  const mappedPillars = pillars.map((p) => ({
    icon: ICON_MAP[p.iconName] || Leaf,
    title: p.title,
    ...
    <PageTransition>
      ...
      <section className="py-12 border-b border-brown/10">
        <Container>
          <StaggerContainer
            staggerDelay={90}
            className="grid items-stretch grid-cols-2 lg:grid-cols-4 gap-6"
          >
            {stats.map((stat) => (
              <div
                key={stat.label}
                className="flex h-full flex-col items-center text-center p-6 rounded-lg bg-ivory-50/80 border border-brown/10 shadow-card"
              >
      ...
      <section className="py-12 sm:py-16 bg-ivory-100 border-y border-brown/10">
        <Container className="grid lg:grid-cols-[0.8fr_1.2fr] gap-6 lg:gap-8 items-stretch">
          ...
          <FadeIn direction="right" className="flex flex-col gap-4 rounded-xl border border-brown/15 bg-white p-7 sm:p-9 shadow-soft text-sm sm:text-base text-brown/75 leading-relaxed">
            {originStory.paragraphs.map((para, i) => (
              <p key={i}>{para}</p>
            ))}
          </FadeIn>
        </Container>
